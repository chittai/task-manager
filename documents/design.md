...
### 2.X タスクとプロジェクトの紐付け (Issue #33)

#### 2.X.1 概要
タスクを特定のプロジェクトに関連付ける機能。ユーザーはタスクを作成または編集する際に、既存のプロジェクトを選択できる。

#### 2.X.2 要件
- タスク作成・編集モーダルに、プロジェクトを選択するためのドロップダウンリストを追加する。
- プロジェクトドロップダウンには、既存のプロジェクト名の一覧が表示される。
- タスクデータには、選択されたプロジェクトのID (`projectId`) を保存する。
- (任意) タスクデータには、表示用にプロジェクト名 (`projectName`) も保持する。
- プロジェクトが選択されていないタスクも許容する (プロジェクトは任意)。

#### 2.X.3 データモデルの変更
- `Task` インターフェース (`src/models/Task.ts`):
    - `projectId?: string;` を追加。
    - `projectName?: string;` を追加。
- `InternalTask` インターフェース (`src/hooks/useTasks.ts`):
    - `projectId?: string;` を追加。

#### 2.X.4 UI変更点
- `TaskModal.tsx` (またはタスクフォームコンポーネント):
    - プロジェクト選択のための `Select` コンポーネントを追加する。
    - `Select` のオプションには、`useProjects` フックから取得したプロジェクト一覧を表示する。

#### 2.X.5 新規フック
- `src/hooks/useProjects.ts`:
    - プロジェクトデータの一覧取得と状態管理を行う。
    - 初期データとしていくつかのモックプロジェクトを持つ。
    - (将来的にはAPI連携)
...
interface Task {
  ...
  projectName?: string; // 関連プロジェクトの名前 (オプション)
                    ...
                    *   `content`:
                        ...
                        *   `CommentList` コンポーネント:
                            ...
                            *   編集・削除機能: 各コメントに編集ボタンおよび削除ボタンを配置する。ユーザーは自身のコメントを編集・削除できる。
                                ...
                                *   **編集時**: 編集ボタンクリックで、対象コメントの内容が入力された編集フォーム（モーダルダイアログまたはインライン展開されるフォーム）を表示し、更新を可能にする。
