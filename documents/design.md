...
-   FR-013.1: タスク一覧の各タスクアイテムから、GTDフローを開始するための「GTD」ボタンを提供すること。
-   FR-013.2: 「GTD」ボタンをクリックすると、該当タスクのIDを `GtdFlowModal` に渡し、モーダルを表示すること。
...
-   FR-020: プロジェクトを作成、読み取り(個別・一覧)、更新、削除できること (基本的なCRUD操作)。localStorage を使用して永続化する。
...
-   FR-028: プロジェクト詳細ページ (`/projects/:projectId`) を提供し、以下の機能を含むこと: (Issue #26)
    -   URLパラメータからプロジェクトIDを取得する。
    -   指定されたプロジェクトの詳細情報（タイトル、説明、期限、重要度など）を表示する。
    -   指定されたプロジェクトに紐づくタスクの一覧を表示する。
    -   プロジェクト情報編集ボタンを設け、クリックで `ProjectModal` を編集モードで開く。
    -   UIはCloudscape Design Systemのガイドラインに準拠する。
...
### 2.10. GTDメソッドに基づくタスク分類システムの拡充

-   FR-031: タスクモデルに以下のGTD関連属性を追加すること。
    -   `delegatedTo`: 委任先の名前/メールアドレス（委任タスク用）
    -   `waitingFor`: 待機中の理由/人（連絡待ちタスク用）
    -   `contextTag`: タスク実行のコンテキスト（場所、ツール、状況など）
    -   `isProject`: プロジェクトフラグ（複数のアクションを含むタスク）
    -   `nextAction`: 次のアクションフラグ（すぐに実行すべきアクション）
    -   `energy`: 必要なエネルギーレベル（「低」「中」「高」）
    -   `time`: 所要時間の目安（「短」「中」「長」）
-   FR-032: タスクフォーム（TaskForm.tsx）に新しいGTD関連属性の編集機能を追加すること。
    -   委任先の入力フィールド
    -   待機理由の入力フィールド
    -   コンテキストタグの選択/入力機能
    -   プロジェクトフラグのチェックボックス
    ...
    -   エネルギーレベルの選択機能
    -   所要時間の選択機能
-   FR-033: タスクカード（TaskCard.tsx）に新しいGTD関連属性の表示機能を追加すること。
    -   委任先情報の表示
    -   待機理由の表示
    -   コンテキストタグのバッジ表示
    -   プロジェクトフラグのアイコン表示
    ...
    -   エネルギーレベルの表示
    -   所要時間の表示
-   FR-034: GTDフローモーダル（GtdFlowModal.tsx）を拡張し、新しいGTD関連属性の設定をサポートすること。
    -   委任先の入力ステップの追加（ステップ5の拡張）
    -   待機理由の入力ステップの追加（ステップ5の拡張）
    -   コンテキストタグの選択ステップの追加
    -   エネルギーレベルと所要時間の設定ステップの追加
-   FR-035: タスク一覧に新しいフィルタリングオプションを追加すること。
    -   コンテキストタグによるフィルタリング
    -   委任先によるフィルタリング
    -   次のアクションフラグによるフィルタリング
    -   エネルギーレベルによるフィルタリング
    -   所要時間によるフィルタリング
-   FR-036: 新しいビュー「次のアクションリスト」を追加し、`nextAction`フラグがセットされたタスクを表示すること。
-   FR-037: コンテキストベースのビューを追加し、選択したコンテキストに関連するタスクを表示すること。
-   FR-038: GTDフロー処理結果とタスク分類システムを連携させること。(Issue #81)
    -   GTDフロー処理の各ステップでの選択結果に基づいて、適切なGTD関連属性を自動設定すること。
      -   プロジェクトとして処理する場合は `isProject` を `true` に設定
      -   委任する場合は `delegatedTo` と `waitingFor` を適切に設定
      -   次のアクションとして処理する場合は `nextAction` を `true` に設定
    -   GTDフロー処理完了後、タスクの新しいステータスに基づいて適切なビューに自動リダイレクトすること。
      -   例：タスクが参考資料に分類された場合、資料リストビューにリダイレクト
    -   タスク分類変更時に成功メッセージを表示し、タスクカードのUI表示を新しい分類に合わせて更新すること。
...
interface Task {
  ...
  // GTDメソッドに基づく拡張属性
  delegatedTo?: string; // 委任先の名前/メールアドレス（委任タスク用）
  waitingFor?: string; // 待機中の理由/人（連絡待ちタスク用）
  contextTag?: string[]; // タスク実行のコンテキスト（場所、ツール、状況など）
  isProject?: boolean; // プロジェクトフラグ（複数のアクションを含むタスク）
  nextAction?: boolean; // 次のアクションフラグ（すぐに実行すべきアクション）
  energy?: 'low' | 'medium' | 'high'; // 必要なエネルギーレベル
  time?: 'quick' | 'medium' | 'long'; // 所要時間の目安
}
```
...
-   `useTasks.ts`: タスクデータに関する状態管理とCRUD操作（LocalStorage利用）を提供します。指定された `projectId` を持つタスクのみをフィルタリングする機能を含みます。(Issue #2, #26 で関連)
-   `useProjects.ts`: プロジェクトデータに関する状態管理とCRUD操作（プロジェクトの新規作成、一覧取得、個別取得 (`getProjectById`)、更新、削除など）を localStorage を利用して提供します。(Issue #23, #26 で関連)

### 4.3. UIコンポーネント (src/components/)

-   `TaskList.tsx`: タスク一覧の表示と操作を行います。各タスクアイテムには、そのタスクに対するGTDフローを開始するための「GTD」ボタンが表示される。このボタンをクリックすると、`GtdFlowModal` が対象タスクのIDと共に呼び出される。
-   `TaskForm.tsx`: タスクの追加・編集フォームを提供します。Issue #4 により、プロジェクト選択ドロップダウンが追加され、新規タスク作成時のデフォルトステータスが `'inbox'` に設定されるよう変更されました。また、既存タスク編集時にもプロジェクトとステータス（`'inbox'`, `'wait-on'` を含む全定義済みステータス）が選択可能です。
...
### 4.5. ページコンポーネント (src/pages/)

-   `ProjectDetailPage.tsx`: 特定のプロジェクトの詳細情報と関連タスク一覧を表示するページコンポーネント。(Issue #26)
    -   目的: ユーザーが選択したプロジェクトに関する包括的な情報を提供し、関連タスクの確認とプロジェクト編集へのアクセスを容易にする。
    -   機能:
        -   URLパラメータ (`/projects/:projectId`) からプロジェクトIDを抽出する。
        -   `useProjects` フックの `getProjectById` を使用して、該当するプロジェクトの詳細情報を取得・表示する。
        -   `useTasks` フックを利用（または拡張）して、取得したプロジェクトIDに紐づくタスクのみをフィルタリングし、一覧表示する（`TaskList` コンポーネントの再利用を検討）。
        -   プロジェクト編集ボタンを配置し、クリックすると `ProjectModal` を編集モードで開き、対象プロジェクトの情報を渡す。
        -   プロジェクトやタスクが見つからない場合のエラーハンドリングまたは適切な表示を行う。
    -   UI は Cloudscape Design System のガイドラインに準拠する。
    -   関連フック: `useProjects`, `useTasks`
    -   関連コンポーネント: `TaskList`, `ProjectModal`
...
-   各ステータス専用のページコンポーネント:
    -   `InboxListPage.tsx`: インボックス内のタスクを表示・管理するページ (`/inbox`)。以下の機能を提供します：
        ...
        -   各タスクに「GTD」ボタンを表示し、GTDフロー処理を開始可能
    -   `WaitingOnListPage.tsx`: 待機中（他者からの返信待ちなど）のタスクを表示・管理するページ (`/waiting-on`)。以下の機能を提供します：
        -   コンポーネントマウント時に'wait-on'ステータスのタスクのみをフィルタリングして表示
        -   タスクの編集・削除・ステータス変更機能
        -   各タスクに「GTD」ボタンを表示し、GTDフロー処理を開始可能
    -   `SomedayMaybeListPage.tsx`: 「いつかやる/多分やる」タスクを表示・管理するページ (`/someday-maybe`)。以下の機能を提供します：
        -   コンポーネントマウント時に'someday-maybe'ステータスのタスクのみをフィルタリングして表示
        -   タスクの編集・削除・ステータス変更機能
        -   各タスクに「GTD」ボタンを表示し、GTDフロー処理を開始可能
    -   `ReferenceListPage.tsx`: 参照資料として保存されたタスクを表示・管理するページ (`/reference`)。以下の機能を提供します：
        -   コンポーネントマウント時に'reference'ステータスのタスクのみをフィルタリングして表示
        -   タスクの編集・削除・ステータス変更機能
        -   各タスクに「GTD」ボタンを表示し、GTDフロー処理を開始可能
...
-   `GtdFlowModal.tsx` コンポーネントは、GTDメソッドに基づいたタスク処理フローを提供します。
-   主な機能:
    -   既存タスクの処理: タスク一覧から「GTD」ボタンをクリックして開始
    -   新規タスクの処理: 新規作成時にGTDフローを適用可能
    -   ステップ1: タスクの名前と説明を入力（「それは何か？」）
    -   ステップ2: 行動の必要性を判断（「行動を起こす必要があるか？」）
    -   ステップ2A: 行動不要な場合の処理（「ゴミ箱」「いつかやる」「参考資料」）
    -   ステップ3: アクション数の判断（「次のアクションは1つか複数か？」）
    -   ステップ4: 2分ルールの適用（「そのアクションは2分以内でできますか？」）
    -   ステップ5: タスクの委任判断（「自分でやるべきか、誰かに任せるか？」）
    -   ステップ6: 日時の特定（「特定の日時が決まっているか？」）
-   処理結果に応じたタスクの状態変更:
    -   ゴミ箱: タスクを削除
    -   いつかやる: タスクのステータスを 'someday-maybe' に変更
    -   参考資料: タスクのステータスを 'reference' に変更
    -   プロジェクト: タスクをプロジェクトに変換
    -   2分ルール適用: タスクのステータスを 'todo' に変更し、すぐに実行するよう促す
    -   委任: タスクのステータスを 'wait-on' に変更し、委任先情報を記録
    -   日時特定あり: タスクのステータスを 'todo' に変更し、期限日を設定
    -   日時特定なし: タスクのステータスを 'todo' に変更し、次のアクションリストに追加
-   新規タスク作成時のGTDフロー処理:
    -   フロー完了時に適切なステータスと属性を持つタスクを作成
    -   作成したタスクを対応するビュー（インボックス、待機中、いつかやる、参考資料）に表示
